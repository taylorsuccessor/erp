<?php include('include/header.php');?>
 ...
 if(isset($_GET['edit_id'])){?>
                    <table class="table" style="display:none;"><tr><td><input type="checkbox" id="<?=$_GET['edit_id'];?>" checked></td></tr></table>
                    <?php
   //  echo great_status(array('','check all','uncheck all','pause','play','stop','agree','disagree','need more info','delete'));
  echo great_status('add_leaving.php','leaving',array('agree','disagree','need more info','delete'));
...
?>
          ...
          function send_data(button_node){
              ...
              my_ajax("add_leaving_php.php",sended_data,button_node,function(data){
          ...
          function send_edit_data(button_node,edit_id){
              ...
              my_ajax("add_leaving_php.php",sended_data,button_node,function(data){alert(data);
                  var data_arr=new Array();data_arr=data.split("||=>||");
...
if(isset($_GET['edit_id'])){
    $edit_result=mysqli_query($con,'select * from leaving where id='.$_GET['edit_id'])or die(mysqli_error($con));
    $edit_row=mysqli_fetch_array($edit_result);
   
    
     $date=$edit_row['date'];
    $from_hour=  decode_time($edit_row['from_hour']);
    $to_hour=  decode_time($edit_row['to_hour']);
    $note=$edit_row['note'];
...
$inputs_arr=    array(
   ...
   array('type'=>'date','name'=>'date','value'=>$date,'label'=>'leaving date'),
    
    array('type'=>'time_pick','name'=>'from_hour','value'=>$from_hour,'label'=>'leaving start hour'),
    array('type'=>'time_pick','name'=>'to_hour','value'=>$to_hour,'label'=>'leaving to hour'),
...
if(isset($_GET['edit_id'])){
   ...
   $inputs_arr[count($inputs_arr)]=array('type'=>'submit','button_text'=>'send to manager','onclick'=>'send_data($(this));');
}//select edit or send
