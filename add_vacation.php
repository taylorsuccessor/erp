<?php include('include/header.php');?>
 ...
 if(isset($_GET['edit_id'])){?>
  ...
  echo great_status('add_vacation.php','vacation',array('agree','disagree','need more info','delete'));
...
?>
          ...
          function send_data(button_node){
              ...
              my_ajax("add_vacation_php.php",sended_data,button_node,function(data){
          ...
          function send_edit_data(button_node,edit_id){
              ...
              my_ajax("add_vacation_php.php",sended_data,button_node,function(data){alert(data);
                  var data_arr=new Array();data_arr=data.split("||=>||");
...
if(isset($_GET['edit_id'])){
    $edit_result=mysqli_query($con,'select * from vacation where id='.$_GET['edit_id'])or die(mysqli_error($con));
    $edit_row=mysqli_fetch_array($edit_result);
   
    $from_date=$edit_row['from_date']; 
    
    $to_date=$edit_row['to_date'];
 $note=$edit_row['note'];
...
$inputs_arr=    array(
   ...
   array('type'=>'date_range_pick','name'=>'date','value'=>'| '.$from_date.' | - | '.$to_date.' |','label'=>'leaving date'),
